...
const AuthScreen: React.FC<AuthScreenProps> = ({ users, onLogin, onSignup }) => {
    ...
    return (
      <div className="min-h-screen bg-[#0b1120] flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="animate-scale-in w-full max-w-sm">
          <div className="bg-[#111827] border border-white/8 rounded-3xl shadow-2xl overflow-hidden">
            <div className="px-8 py-10 text-center">
              <div className="w-16 h-16 bg-amber-500/15 border border-amber-500/20 rounded-2xl flex items-center justify-center mx-auto mb-6">
                <svg className="w-8 h-8 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
              </div>
              <h2 className="text-xl font-bold text-white mb-2">Inscription en attente</h2>
              <p className="text-slate-400 text-sm leading-relaxed mb-6">
                Votre inscription est en attente d'approbation. Contactez le créateur de l'application :
              </p>
              <div className="bg-[#345d6e]/15 border border-[#345d6e]/25 rounded-2xl px-6 py-4 mb-8">
                <p className="text-[#7ecde8] font-black text-2xl tracking-[0.15em]">06 99 40 70 36</p>
              </div>
              <button
                onClick={() => { setShowSignupPending(false); setIsLoginMode(true); setName(''); setSurname(''); setPassword(''); }}
                className="btn-press w-full bg-[#345d6e] hover:bg-[#2c5263] text-white font-semibold py-3 rounded-xl transition-colors text-sm"
              >
                Retour à la connexion
              </button>
            </div>
            <div className="px-8 py-4 border-t border-white/5 text-center">
              <p className="text-[10px] text-slate-600">{APP_CREDITS}</p>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#0b1120] flex overflow-hidden relative">
      {/* Background glow orbs */}
      <div className="absolute top-[-15%] right-[-10%] w-[700px] h-[700px] bg-[#345d6e]/10 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-[-20%] left-[-10%] w-[600px] h-[600px] bg-blue-900/15 rounded-full blur-3xl pointer-events-none" />

      {/* Left panel — branding */}
      <div className="hidden lg:flex w-1/2 flex-col justify-between p-14 relative">
        <div>
          <div className="flex items-center gap-3 mb-16">
            <div className="w-9 h-9 bg-[#345d6e] rounded-xl flex items-center justify-center">
            ...
            <div>
              ...
              <p className="text-slate-500 text-[10px] font-medium uppercase tracking-widest">Gestion Materiel HMD</p>
            </div>
          </div>
          ...
          <div className="animate-fade-up">
            <h1 className="text-5xl font-black text-white leading-tight tracking-tight mb-4">
              Plateforme de<br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#7ecde8] to-[#345d6e]">
                Gestion RH
              </span>
            </h1>
          ...
          <div className="mt-14 space-y-5 animate-fade-up stagger-2">
            {[
              ...
              <div key={item.label} className={`flex items-center gap-4 animate-slide-right stagger-${i+2}`}>
                <div className="w-10 h-10 bg-white/5 border border-white/8 rounded-xl flex items-center justify-center shrink-0">
                  <svg className="w-5 h-5 text-[#7ecde8]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex items-center gap-2">
          ...
          <p className="text-slate-600 text-xs font-medium">Système sécurisé — Données stockées localement</p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8">
        <div className="w-full max-w-sm animate-scale-in">
          ...
          <div className="lg:hidden flex items-center gap-3 mb-10 justify-center">
            ...
            <p className="text-white font-black text-sm">CSGM AMROUS</p>
          </div>
          ...
          <div className="bg-[#111827] border border-white/8 rounded-3xl shadow-2xl overflow-hidden">
            <div className="px-8 pt-8 pb-2">
              ...
              {error && (
                <div className="bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-medium px-4 py-3 rounded-xl mb-5 flex items-center gap-2.5">
              ...
              <form onSubmit={isLoginMode ? handleLogin : handleSignup} className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest">Nom</label>
                    <input
                      type="text"
                      value={name}
                      onChange={(e) => { setName(e.target.value.toUpperCase()); setError(''); }}
                      className="premium-input w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 font-semibold text-white placeholder-slate-600 text-sm uppercase"
                      placeholder="NOM"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest">Prénom</label>
                    <input
                      type="text"
                      value={surname}
                      onChange={(e) => { setSurname(e.target.value); setError(''); }}
                      className="premium-input w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 font-semibold text-white placeholder-slate-600 text-sm capitalize"
                      placeholder="Prénom"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest">Mot de passe</label>
                  <div className="relative">
                    <input
                      type={showPassword ? "text" : "password"}
                      value={password}
                      onChange={(e) => { setPassword(e.target.value); setError(''); }}
                      className="premium-input w-full bg-white/5 border border-white/10 rounded-xl pl-4 pr-12 py-3 font-semibold text-white placeholder-slate-600 text-sm"
                      placeholder="••••••••"
                    />
                ...
                <button type="submit"
                  className="btn-press w-full bg-[#345d6e] hover:bg-[#2c5263] text-white font-bold py-3.5 rounded-xl transition-colors text-sm mt-2 shadow-lg shadow-[#345d6e]/25">
              ...
              <div className="mt-6 pb-6 text-center">
                <button onClick={() => { setIsLoginMode(!isLoginMode); setError(''); setName(''); setSurname(''); setPassword(''); }}
                  className="text-xs text-slate-500 hover:text-[#7ecde8] transition-colors font-medium">
                  {isLoginMode ? 'Nouvel utilisateur ? Créer un compte' : 'Déjà un compte ? Se connecter'}
                </button>
              </div>
            </div>
            <div className="px-8 py-4 border-t border-white/5 text-center">
              <p className="text-[10px] text-slate-700">{APP_CREDITS}</p>
            </div>
          </div>
        </div>
      </div>
